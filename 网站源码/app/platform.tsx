...
} }}><Download />下载监测数据</Button></div><Tabs value={view} onValueChange={setView}><div className="monitor-tabs"><TabsList><TabsTrigger value="map">俯瞰图与数据展示</TabsTrigger>{admin && <TabsTrigger value="manage">修改后台数据</TabsTrigger>}</TabsList>{admin && <span>管理账户可在两个工具之间切换 · <a href="/api/assets/dayun-monitor-app-v7.zip?download=1">下载原监测工具包</a></span>}</div><TabsContent value="map"><div className="monitor-surface"><Monitor /></div></TabsContent>{admin && <TabsContent value="manage"><div className="monitor-surface"><MonitorAdmin /></div></TabsContent>}</Tabs></div>; }
