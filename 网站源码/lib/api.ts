...
import {downloadR2File} from './r2-download';
...
async function streamFile(row: any, request: Request) { const inline = new URL(request.url).searchParams.get('download') !== '1' && ['image/png', 'image/jpeg', 'image/webp', 'application/pdf', 'video/mp4'].includes(row.mime); const response = await downloadR2File(bucket(), row.object_key, request, { 'Content-Type': row.mime, 'Content-Disposition': `${inline ? 'inline' : 'attachment'}; filename*=UTF-8''${encodeURIComponent(row.name)}`, 'X-Content-Type-Options': 'nosniff', 'Content-Security-Policy': "sandbox; default-src 'none'; style-src 'unsafe-inline'", 'Cross-Origin-Resource-Policy': 'same-origin' }); if (!response) throw new HttpError(404, '文件暂时不可用，请联系管理人员。'); return response; }
...
export async function handleAPI(request: Request) {
    try {
        ...
        if(parts[0]==='sections'&&parts[1]){
            ...
            if(method==='GET'){if(existing?.archived)throw new HttpError(404,'此断面图已删除，可在修改记录中恢复。');if(!existing||existing.object_key.startsWith('original:'))return await asset('section-'+id+'.png',request);const response=await downloadR2File(bucket(),existing.object_key,request,{'X-Content-Type-Options':'nosniff'});if(!response)throw new HttpError(404,'断面图片不可用。');return response;}
